...
export default function LeaveTab({ userId, companyId }) {
  ...
  async function handleAdd() {
    ...
    const { error } = await addLeave({ start_date: start, end_date: end, reason: reason.trim() || null, company_id: companyId || null })
    if (error) {
  ...
  async function handleDelete(id) {
    ...
    if (error) toast.current.show({ severity: 'error', summary: 'Error', detail: error.message, life: 4000 })
  }
        ...
        <Card className="glass-card maintenance-card maintenance-panel">
          <div className="d-flex flex-column gap-4">
            <div>
              ...
              <p className="section-copy">Select a date range and optionally add a reason. Leave takes effect immediately — no approval required.</p>
            </div>
        ...
        <Card className="glass-card logs-card table-panel">
          <div className="table-header">
            <div className="table-header-copy">
              ...
              <p className="text-muted-soft">All submitted leave entries. You can delete upcoming ones.</p>
            </div>
          </div>
          <DataTable
            value={leaves}
